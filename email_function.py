...
def on_message_received(ch, method, properties, body):
    ...
    cursor.execute("select email_list from omr_data where job_id=%s",(jobid,))
    email_id=list(cursor.fetchall()[0])[0]
    email_ids = set(email_id)
    email_id = list(email_ids)
    for emails in email_id:
	    ...
	    smtp.close()
	    cursor.execute("update omr_data set job_status='done' where job_id=%s",(jobid,))
	    print("Email Sent!")
